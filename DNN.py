...
from tensorflow.contrib.learn import extract_pandas_labels
...
if __name__ == "__main__":
	...
	with tf.Session() as sess:
		...
		def add_layer(inputs,input_dim,output_dim,activation=None,drop_out=False,keep_prob=0.9):
		...
		keep_prob=True
		#Create graphic
		layer1=add_layer(xs,11,30,activation=tf.tanh,drop_out=keep_prob)
		layer2=add_layer(layer1,30,50,activation=tf.tanh,drop_out=keep_prob)
		layer3=add_layer(layer2,50,50,activation=tf.tanh,drop_out=keep_prob)
		layer4=add_layer(layer3,50,50,activation=tf.tanh,drop_out=keep_prob)
		layer5=add_layer(layer4,50,30,activation=tf.tanh,drop_out=keep_prob)
		prediction=add_layer(layer5,30,1,activation=tf.nn.sigmoid)

		#cross_entropy = tf.reduce_mean(-tf.reduce_sum(tf.expand_dims(ys,1)*tf.log(prediction+1e-5)+(1-tf.expand_dims(ys,1))*tf.log(1-prediction+1e-5), reduction_indices=[1]))
		...
		#train_step2 = tf.train.AdamOptimizer(learning_rate=0.001).minimize(cross_entropy)
		...
		#train_step2 = tf.train.MomentumOptimizer(1e-2,0.3).minimize(cross_entropy)
		#train_step3 = tf.train.ProximalAdagradOptimizer(1e-4,l2_regularization_strength=0.01).minimize(cross_entropy)
		#train_step = tf.train.AdamOptimizer(1e-1).minimize(cross_entropy)
		#train_step2 = tf.train.RMSPropOptimizer(1e-3).minimize(cross_entropy)

		sess.run(tf.global_variables_initializer())
		batches = np.split(x_train,300)
		labels = np.split(y_train,300)
		steps = 1000
		with sess.as_default():
		    for i in range(steps):
			    ...
			    if not i%100:
				    ...
				    print "Cross_entropy:", cross_entropy.eval(feed_dict={xs: x_train, ys: y_train}), cross_entropy.eval(feed_dict={xs: x_test, ys: y_test})
		    for i in range(steps):
			    for batch in zip(batches,labels):
				train_step2.run(feed_dict={xs: batch[0],
					 		   ys: batch[1]})
			    if not i%100:
				    print "Cross_entropy2:", cross_entropy.eval(feed_dict={xs: x_train, ys: y_train}), cross_entropy.eval(feed_dict={xs: x_test, ys: y_test})
		    '''
		    for i in range(500):
			    for batch in zip(batches,labels):
				train_step3.run(feed_dict={xs: batch[0],
					 		   ys: batch[1]})
			    if not i%100:
				    print "Cross_entropy3:", cross_entropy.eval(feed_dict={xs: x_train,
											  ys: y_train})
       	            '''             
